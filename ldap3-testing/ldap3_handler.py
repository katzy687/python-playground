...
class LdapHandler:
    ...
    def get_groups_dn_list(self) -> List[str]:
        with self._conn:
    ...
    def get_group_users(self, group_dn: str) -> List[UserData]:
        with self._conn:
            self._conn.search(search_base=group_dn,
                              search_filter='(objectClass=organizationalPerson)',
                              attributes=["*"])
            return [UserData(x.uid.value, x.cn.value, x.mail.value) for x in self._conn.entries]

    def get_groups_table(self) -> Dict[str, List[UserData]]:
        group_dn_list = self.get_groups_dn_list()
        result = {}
...
if __name__ == "__main__":
    ...
    handler = LdapHandler(LDAP_SERVER_URI, LDAP_USER_DN, LDAP_PASSWORD, LDAP_SEARCH_BASE)
